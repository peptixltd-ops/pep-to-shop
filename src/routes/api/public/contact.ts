      ...
      POST: async ({ request }) => {
        ...
        const { error: enqueueError } = await supabase.rpc('enqueue_email', {
          ...
          payload: {
            ...
            reply_to: `${data.name.replace(/[<>"]/g, '')} <${data.email}>`,
            headers: {
              'Reply-To': `${data.name.replace(/[<>"]/g, '')} <${data.email}>`,
              'Message-ID': `<contact-${messageId}@pondokpeptides.com>`,
              'References': `<contact-thread-${data.email.toLowerCase()}@pondokpeptides.com>`,
              'In-Reply-To': `<contact-thread-${data.email.toLowerCase()}@pondokpeptides.com>`,
